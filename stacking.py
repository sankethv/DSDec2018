...
ab3 = ensemble.AdaBoostClassifier(random_state=100)

dt = tree.DecisionTreeClassifier(random_state=100) #For Stacking

stack_estimator = mlxClassifier.StackingClassifier(classifiers=[rf1, ab3], meta_classifier=dt) #, store_train_meta_features=True)
stack_grid = {'randomforestclassifier__n_estimators': [10, 15],
             ...
             # 'gradientboostingclassifier__n_estimators': [5, 10],
              'adaboostclassifier__n_estimators':[5,15,25],
              'adaboostclassifier__random_state':[2017],
              'adaboostclassifier__learning_rate':[0.01,0.02,0.03],
              'meta-decisiontreeclassifier__min_samples_split': [2, 3, 4]} #meta- prefix is required
...
print(grid_stack_estimator.grid_scores_)
...
titanic_test.to_csv('submission_Stacking_adaboost6.csv', columns=['PassengerId','Survived'],index=False)
